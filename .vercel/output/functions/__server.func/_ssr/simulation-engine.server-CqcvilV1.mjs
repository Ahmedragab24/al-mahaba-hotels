...
async function ensureMinCustomers(admin, minCount) {
  const { count } = await admin.from("customers").select("*", { count: "exact", head: true }).eq("is_simulated", true);
  const have = count ?? 0;
...
async function ensureMinSuppliers(admin, minCount) {
  const { count } = await admin.from("suppliers").select("*", { count: "exact", head: true }).eq("is_simulated", true);
  const have = count ?? 0;
  const need = Math.max(0, minCount - have);
  if (need === 0) return;
  const rows = [];
...
async function createRfq(admin) {
  const { data: customers } = await admin.from("customers").select("id").eq("is_simulated", true).limit(50);
  if (!customers || customers.length === 0) return null;
  const customer = pick(customers);
  const startDays = intBetween(7, 60);
  const nights = intBetween(2, 7);
  const start = new Date(Date.now() + startDays * 864e5);
  const end = new Date(start.getTime() + nights * 864e5);
  const fmt = (d) => d.toISOString().slice(0, 10);
  const destination = pick(["الرياض", "جدة", "مكة", "المدينة", "الدمام", "أبها", "تبوك"]);
  const { data, error } = await admin.from("rfqs").insert({
...
async function createBooking(admin) {
  const { data: customers } = await admin.from("customers").select("id, preferred_currency").eq("is_simulated", true).limit(50);
  if (!customers || customers.length === 0) return null;
  const customer = pick(customers);
  const { data, error } = await admin.from("bookings").insert({
    ...
    booking_date: (/* @__PURE__ */ new Date()).toISOString().slice(0, 10),
...
async function createInvoice(admin) {
  const { data: bookings } = await admin.from("bookings").select("id, customer_id, currency").eq("is_simulated", true).limit(50);
  if (!bookings || bookings.length === 0) return null;
  const b = pick(bookings);
  const subtotal = intBetween(500, 15e3);
...
async function createReceipt(admin) {
  const { data: invoices } = await admin.from("invoices").select("id, customer_id, currency, total_amount, paid_amount").eq("is_simulated", true).limit(20);
  if (!invoices || invoices.length === 0) return null;
  const inv = pick(invoices);
  const remaining = Math.max(0, Number(inv.total_amount) - Number(inv.paid_amount || 0));
  if (remaining <= 0) return null;
  const amount = Math.min(remaining, intBetween(100, 5e3));
  const { data, error } = await admin.from("receipts").insert({
    receipt_no: `SIM-RCP-${Date.now().toString(36).toUpperCase()}`,
    customer_id: inv.customer_id,
    currency: inv.currency || "SAR",
    receipt_date: (/* @__PURE__ */ new Date()).toISOString().slice(0, 10),
    amount,
...
async function createRate(admin) {
  const { data: hotels } = await admin.from("hotels").select("id").is("deleted_at", null).limit(50);
  if (!hotels || hotels.length === 0) return null;
  const hotel = pick(hotels);
  const { data: rts } = await admin.from("hotel_room_types").select("id").eq("hotel_id", hotel.id).limit(20);
  if (!rts || rts.length === 0) return null;
  const rt = pick(rts);
  const isDirect = Math.random() < 0.4;
  let supplierId = null;
  if (!isDirect) {
    const { data: sups } = await admin.from("suppliers").select("id").eq("is_simulated", true).limit(30);
    if (!sups || sups.length === 0) return null;
    supplierId = pick(sups).id;
  }
  const today = /* @__PURE__ */ new Date();
  const validFrom = new Date(today.getTime() - 7 * 864e5);
  const validTo = new Date(today.getTime() + 90 * 864e5);
  const cost = intBetween(180, 1400);
  const markup = intBetween(10, 35);
  const selling = Math.round(cost * (1 + markup / 100));
  const fmt = (d) => d.toISOString().slice(0, 10);
  const { data, error } = await admin.from("rates").insert({
...
async function runSimulationTick(opts = {}) {
  const { supabaseAdmin } = await import("./client.server-D5ro3rAQ.mjs");
  const admin = supabaseAdmin;
  const { data: settings } = await admin.from("simulation_settings").select("*").limit(1).maybeSingle();
  if (!settings) {
  ...
  try {
    ...
    await admin.from("simulation_settings").update({
    ...
    return { ok: true, created, intensity, ts: (/* @__PURE__ */ new Date()).toISOString() };
  } catch (e) {
