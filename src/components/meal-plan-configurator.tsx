  ...
  return (
    <>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8" dir={lang === "ar" ? "rtl" : "ltr"}>
        <div
          className={cn("border rounded-xl p-6 cursor-pointer flex justify-between items-center transition-colors", mealsIncluded ? "border-amber-600 bg-amber-50/10" : "border-border hover:border-amber-600/50")}
          onClick={() => onMealsIncludedChange(true)}
        >
          <div>
            <div className="font-bold text-lg mb-1">{lang === "ar" ? "شامل" : "Included"}</div>
            <div className="text-sm text-muted-foreground">
              {lang === "ar" ? "الوجبات مشمولة في سعر الحجز" : "Meals are included in the booking rate"}
            </div>
          </div>
          <div className={cn("w-5 h-5 rounded-full border-2 flex items-center justify-center", mealsIncluded ? "border-amber-600" : "border-muted-foreground/30")}>
        ...
        >
          <div>
            <div className="font-bold text-lg mb-1">{lang === "ar" ? "غير شامل" : "Not Included"}</div>
            <div className="text-sm text-muted-foreground">
      ...
      {mealsIncluded ? (
        ...
        <div className="border rounded-xl overflow-hidden max-w-3xl mx-auto">
          <div className="bg-amber-50/30 px-6 py-4 flex justify-between items-center border-b text-sm font-bold text-muted-foreground" dir={lang === "ar" ? "rtl" : "ltr"}>
          ...
          <div className="divide-y" dir={lang === "ar" ? "rtl" : "ltr"}>
            {[
              ...
              { id: "HB", label_ar: "نص إقامة", label_en: "Half Board", field: "half_board_price" as const },
              { id: "FB", label_ar: "إقامة كاملة", label_en: "Full Board", field: "full_board_price" as const },
            ].map(meal => {
              const isChecked = mealPlanComponents.includes(meal.id);
              return (
                <div key={meal.id} className="p-4 flex items-center justify-between hover:bg-muted/10 transition-colors">
                  <div className="flex items-center gap-4">
                    <div
                      ...
                      className={cn("w-4 h-4 rounded flex items-center justify-center border cursor-pointer", isChecked ? "bg-amber-600 border-amber-600 text-white" : "border-muted-foreground/30 bg-white dark:bg-transparent")}
                    >
                    ...
                    {isChecked ? (
                      <div className="relative">
                        ...
                        <Input
                          ...
                          min="0"
                          placeholder="0.00"
                          className="h-10 text-right pr-3 pl-10 border-amber-200 focus-visible:ring-amber-600"
