...
} from "@/store/services/customers/customersService";
...
function CustomersList() {
  ...
  const metricsData = useMemo(() => {
    ...
    return {
      ...
      thisMonth: rows.filter((r: any) => new Date(r.created_at) >= monthStart && !r.is_archived)
        .length,
    };
  }, [allCustomers]);
  ...
  if (Array.isArray(listData)) actualListData = listData;
    ...
    actualListData = Array.isArray(dataObj)
      ? dataObj
      : Array.isArray(dataObj.data)
        ? dataObj.data
        : Array.isArray(dataObj.items)
              ...
              <TableBody>
                ...
                {pagedRows.map((c: any) => (
                  <TableRow key={c.id} className={c.is_archived ? "opacity-60" : ""}>
                    ...
                    <TableCell>
                      {c.type === "individual" && (
                        <Badge className="bg-blue-100 text-blue-800 hover:bg-blue-200 dark:bg-blue-500/20 dark:text-blue-400 dark:hover:bg-blue-500/30 border-transparent">
                          {t(`ctype.${c.type}`)}
                        </Badge>
                      )}
                      {c.type === "company" && (
                        <Badge className="bg-purple-100 text-purple-800 hover:bg-purple-200 dark:bg-purple-500/20 dark:text-purple-400 dark:hover:bg-purple-500/30 border-transparent">
                          {t(`ctype.${c.type}`)}
                        </Badge>
                      )}
                      {c.type === "agency" && (
                        <Badge className="bg-amber-100 text-amber-800 hover:bg-amber-200 dark:bg-amber-500/20 dark:text-amber-400 dark:hover:bg-amber-500/30 border-transparent">
                          {t(`ctype.${c.type}`)}
                        </Badge>
                      )}
                      {c.type === "government" && (
                        <Badge className="bg-emerald-100 text-emerald-800 hover:bg-emerald-200 dark:bg-emerald-500/20 dark:text-emerald-400 dark:hover:bg-emerald-500/30 border-transparent">
                          {t(`ctype.${c.type}`)}
                        </Badge>
                      )}
                      {!["individual", "company", "agency", "government"].includes(c.type) && (
                        <Badge variant="outline">{t(`ctype.${c.type}`)}</Badge>
                      )}
                    </TableCell>
                    <TableCell className="text-xs">{c.email}</TableCell>
                    <TableCell className="text-xs">{c.phone || "-"}</TableCell>
                    <TableCell className="text-xs">
                      {c.country?.code || c.country_id || "-"}
                    </TableCell>
                    <TableCell>
                      <StatusBadge status={c.status ? "active" : "inactive"} />
                    </TableCell>
                      ...
                      <div className="flex justify-end gap-1">
                        ...
                        {isAdmin(auth) && !c.is_archived && (
                          <Button
                            ...
                            onClick={() => setConfirmId({ id: c.id, action: "archive" })}
                          >
                        ...
                        {isAdmin(auth) && c.is_archived && (
                          <Button
                            ...
                            onClick={() => setConfirmId({ id: c.id, action: "restore" })}
                          >
                        ...
                        {isAdmin(auth) && c.is_archived && (
                          <Button
                            ...
                            onClick={() => setConfirmId({ id: c.id, action: "delete" })}
                          >
