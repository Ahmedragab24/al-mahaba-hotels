...
export default function HotelsList() {
  ...
  const countriesList = Array.isArray(countries.data)
    ? countries.data
    : (countries.data as any)?.data || [];
  ...
  const hotelsList = useMemo(() => {
    return Array.isArray(allHotels)
      ? allHotels
      : Array.isArray((allHotels as any)?.data)
        ? (allHotels as any).data
        : [];
  }, [allHotels]);
        ...
        <Card>
          <CardContent className="grid grid-cols-1 gap-3 p-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
            ...
            <div className="flex flex-col gap-1.5">
              ...
              >
                <SelectTrigger className="w-full">
                  <SelectValue placeholder={t("label.stars")} />
                </SelectTrigger>
                <SelectContent>
                  <SelectItem value="all">{t("filter.all")}</SelectItem>
                  {STARS.map((n) => (
        ...
        <div className="grid grid-cols-1 gap-5 sm:grid-cols-2 xl:grid-cols-3">
          {list.data?.rows.map((h: any) => {
            ...
            const cityName = h.city
              ? lang === "ar"
                ? h.city.name_ar || h.city.name_en
                : h.city.name_en || h.city.name_ar
              : null;
            const countryName = h.country
              ? lang === "ar"
                ? h.country.name_ar || h.country.name_en
                : h.country.name_en || h.country.name_ar
              : null;
              ...
              >
                ...
                >
                  ...
                  <div className="absolute top-3 start-3 flex items-center gap-2">
                    <StatusBadge status={h.status ? "active" : "inactive"} />
                  </div>
                  {h.stars ? (
                      ...
                      {isAdmin(auth) &&
                        (h.is_archived ? (
                          <Button
                            ...
                            onClick={() => setConfirm({ id: h.id, action: "restore" })}
                          >
                          ...
                          <Button
                            ...
                            onClick={() => setConfirm({ id: h.id, action: "archive" })}
                          >
