...
export default function PaymentsList() {
  ...
  const list = useQuery({
    ...
    queryFn: async () => {
      ...
      // Filter locally for fully paid balance (paid >= total)
  ...
  // Aggregate metrics
  const totalRemaining = allRows.reduce((sum, b) => sum + Math.max(Number(b.total_amount) - Number(b.amount_paid), 0), 0);
  ...
  return (
    <>
      <PageHeader
        ...
        subtitle={ar("متابعة المدفوعات المالية المسددة بالكامل", "Track fully paid financial payments")}
      />
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          <Card className="bg-red-50 dark:bg-red-950/20 border-red-200 dark:border-red-900/30">
            <CardContent className="p-5 flex items-center gap-4">
              ...
              <div>
                <p className="text-sm font-medium text-red-600/80 dark:text-red-400/80 uppercase tracking-wider">{ar("إجمالي المدفوعات (المتبقي)", "Total Payments (Remaining)")}</p>
                <p className="text-2xl font-bold text-red-700 dark:text-red-300 tabular-nums mt-1">{fmt(totalRemaining)}</p>
              </div>
            </CardContent>
          </Card>
          <Card className="bg-blue-50 dark:bg-blue-950/20 border-blue-200 dark:border-blue-900/30">
            <CardContent className="p-5 flex items-center gap-4">
              ...
              <div>
                <p className="text-sm font-medium text-blue-600/80 dark:text-blue-400/80 uppercase tracking-wider">{ar("إجمالي المدفوعات", "Total Payments")}</p>
          ...
          <Card className="bg-emerald-50 dark:bg-emerald-950/20 border-emerald-200 dark:border-emerald-900/30">
            <CardContent className="p-5 flex items-center gap-4">
              ...
              <div>
                <p className="text-sm font-medium text-emerald-600/80 dark:text-emerald-400/80 uppercase tracking-wider">{ar("المدفوع فعلياً", "Actually Paid")}</p>
            ...
            <Table>
              <TableHeader>
                <TableRow className="whitespace-nowrap bg-muted/40 hover:bg-muted/40">
                  ...
                  <TableHead className="font-bold text-red-600">{ar("المتبقي (المدفوع)", "Remaining (Paid)")}</TableHead>
                  ...
                  <TableRow>
                    <TableCell colSpan={8} className="py-16 text-center text-muted-foreground">
                      <div className="flex flex-col items-center justify-center gap-2">
                        ...
                        <p>{ar("لا توجد مبالغ مدفوعة", "No paid amounts")}</p>
                      </div>
                    </TableCell>
                  </TableRow>
                )}
