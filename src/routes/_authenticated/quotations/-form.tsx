...
import { User, Globe, Building2, BedDouble, CalendarDays, FileText, Star, Trash2 } from "lucide-react";
...
const BOARDS = [
  ...
  { code: "HB", label: { ar: "نصف إقامة (HB)", en: "Half Board (HB)" } },
  { code: "FB", label: { ar: "إقامة كاملة (FB)", en: "Full Board (FB)" } },
  ...
  return (
    <div className="space-y-5">
              ...
              {selectedItems.length > 0 && (
                <div className="border rounded-md overflow-hidden bg-card shadow-sm">
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="bg-muted/50 text-muted-foreground border-b text-center">
                        <th className="py-2 px-3 text-start">الغرفة</th>
                        <th className="py-2 px-3">الإطلالة</th>
                        <th className="py-2 px-3">خطة الوجبات</th>
                        <th className="py-2 px-3">سعر الليلة</th>
                    ...
                    <tbody className="divide-y text-center">
                      {selectedItems.map((item, idx) => (
                        <tr key={idx} className="hover:bg-muted/30">
                          <td className="py-2 px-3 text-start font-medium">{lang === "ar" ? item.room_name_ar : item.room_name_en}</td>
                          <td className="py-2 px-3">{item.view_id ? (lang === "ar" ? item.view_name_ar : item.view_name_en) : "—"}</td>
                          <td className="py-2 px-3"><Badge variant="outline" className="bg-background">{t(`board.${item.meal_plan}`)}</Badge></td>
                          <td className="py-2 px-3 text-amber-700 dark:text-amber-500 font-bold">{item.selling_price} {form.currency}</td>
                          <td className="py-2 px-3 font-medium">{item.rooms}</td>
                          <td className="py-2 px-3 text-end">
                            <Button variant="ghost" size="icon" onClick={() => setSelectedItems(prev => prev.filter((_, i) => i !== idx))}>
      ...
      <QuotationRatesDialog
        ...
        onSave={(items) => setSelectedItems(prev => [...prev, ...items])}
      />
    </div>
  );
}
