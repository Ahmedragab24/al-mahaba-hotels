...
export function OccupancyTab({ rateId, currency, canWrite }: { rateId: string; currency: string; canWrite: boolean }) {
  const { t } = useI18n();
  const qc = useQueryClient();
  const [open, setOpen] = useState(false);
  const [editId, setEditId] = useState<string | null>(null);
  const [delId, setDelId] = useState<string | null>(null);
  const blank = { occupancy_type: "" as string, cost_price: "" as any, markup_percent: "" as any, selling_price: "" as any, active: true };
  ...
  const del = useMutation({
    ...
    onSuccess: () => { toast.success(t("toast.deleted")); qc.invalidateQueries({ queryKey: ["rate-occupancy", rateId] }); setDelId(null); },
      ...
      <CardContent className="space-y-3 p-4">
        ...
        {canWrite && (
          <div className="flex justify-end">
            <Button size="sm" disabled={used.size >= OCCUPANCY_TYPES.length}
        ...
        <Table>
          <TableHeader><TableRow className="whitespace-nowrap">
          ...
          <TableBody>
            {list.isLoading && <TableRow><TableCell colSpan={7} className="py-6 text-center text-muted-foreground">{t("label.loading")}</TableCell></TableRow>}
            {!list.isLoading && (list.data?.length ?? 0) === 0 && (
              <TableRow><TableCell colSpan={7} className="py-6 text-center text-muted-foreground">{t("label.no_results")}</TableCell></TableRow>
            )}
            {list.data?.map((r) => (
              <TableRow key={r.id} className="whitespace-nowrap">
                <TableCell className="font-medium">{t(`occupancy.${r.occupancy_type}`)}</TableCell>
                <TableCell className="text-end font-mono">{Number(r.cost_price).toFixed(2)}</TableCell>
                <TableCell className="text-end font-mono">{r.markup_percent != null ? Number(r.markup_percent).toFixed(2) + " %" : "—"}</TableCell>
                <TableCell className="text-end font-mono">{r.selling_price != null ? Number(r.selling_price).toFixed(2) : "—"}</TableCell>
                ...
                <TableCell className="text-end">
                  {canWrite && <>
                    <Button variant="ghost" size="icon" title={t("actions.edit")} onClick={() => {
                      setEditId(r.id);
                      setForm({
                    ...
                    <Button variant="ghost" size="icon" title={t("actions.delete")} onClick={() => setDelId(r.id)}>
        ...
        <Dialog open={open} onOpenChange={(v) => { setOpen(v); if (!v) { setEditId(null); setForm(blank); } }}>
          <DialogContent>
            <DialogHeader><DialogTitle>{t("rates.tab.occupancy")}</DialogTitle></DialogHeader>
            <div className="grid grid-cols-2 gap-3">
              <div className="col-span-2">
                <label className="text-xs">{t("occupancy.type")} *</label>
                <Select value={form.occupancy_type} onValueChange={(v) => setForm({ ...form, occupancy_type: v })} disabled={!!editId}>
                  <SelectTrigger className="w-full"><SelectValue placeholder={t("occupancy.type")} /></SelectTrigger>
                  <SelectContent>
                    {OCCUPANCY_TYPES.map((o) => (
                      <SelectItem key={o} value={o} disabled={used.has(o)}>{t(`occupancy.${o}`)}</SelectItem>
                    ))}
                  </SelectContent>
                </Select>
              </div>
              <div><label className="text-xs">{t("rates.cost")} *</label>
                <Input type="number" step="0.01" min="0" value={form.cost_price} onChange={(e) => onCost(e.target.value)} /></div>
              <div><label className="text-xs">{t("rates.markup")}</label>
                <Input type="number" step="0.01" min="0" value={form.markup_percent} onChange={(e) => onMarkup(e.target.value)} /></div>
              <div><label className="text-xs">{t("rates.selling")}</label>
                <Input type="number" step="0.01" min="0" value={form.selling_price} onChange={(e) => onSelling(e.target.value)} /></div>
