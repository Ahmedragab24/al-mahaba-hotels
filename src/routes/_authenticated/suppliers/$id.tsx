...
export default function SupplierDetail() {
  ...
  if (error) return <div className="p-6 text-red-500">Error: {(error as any)?.message || t("label.error")}</div>;
  ...
  return (
    <>
      ...
      <div className="p-6">
        {editing ? (
          <SupplierForm initial={s} onSaved={() => { setEditing(false); qc.invalidateQueries({ queryKey: ["Suppliers"] }); }} />
        ) : (
          <Card><CardContent className="grid gap-3 p-6 md:grid-cols-3 text-sm">
            <KV label={t("label.code")} value={s.code} mono />
            <KV label={t("filter.type")} value={lang === "ar" ? (s.supplier_type?.name_ar ?? s.supplier_type?.name_en) : (s.supplier_type?.name_en ?? s.supplier_type?.name_ar)} />
            <KV label={t("label.status")} value={s.status ? t("status.active") : t("status.inactive")} />
            ...
            <KV label={t("label.currency")} value={s.currency ? `${s.currency.code} ${lang === "ar" ? (s.currency.symbol_ar ?? s.currency.symbol_en) : (s.currency.symbol_en ?? s.currency.symbol_ar)}` : ""} />
            ...
            <KV label={t("label.address")} value={[s.address_1, s.address_2].filter(Boolean).join(", ")} />
            <KV label={t("label.phone")} value={s.phone ?? ""} />

            <KV label={t("label.email")} value={s.email ?? ""} />
            <KV label={t("label.website")} value={s.website ?? ""} />
            <KV label={t("label.created_at")} value={new Date(s.created_at).toLocaleDateString(lang)} />
            <KV label={t("label.updated_at")} value={new Date(s.updated_at).toLocaleDateString(lang)} />
